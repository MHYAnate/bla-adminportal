...
const CustomersDataTable: React.FC<iProps> = ({ data, loading }) => {
	...
	const handleStatusUpdate = (
		...
		console.log(
			`Dashboard: Customer ${customerId} status updated to ${newStatus}`
		);
	...
	const cellRenderers = {
        ...
        /> */}
				...
				<div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center text-sm font-semibold text-white shadow-md">
					{item?.name
						? item.name
								.split(" ")
								.map((word) => word[0])
		...
		type: (item: CustomersData) => (
			<span className="font-medium">
				{capitalizeFirstLetter(
					item?.customerType?.toString() || item?.type?.toString() || "customer"
				)}
			</span>
		),
		...
		status: (item: CustomersData) => {
			...
			if (!item?.id) {
				const customerStatus =
					item?.customerStatus || item?.status || "INACTIVE";
				return (
					<Badge
						variant={
