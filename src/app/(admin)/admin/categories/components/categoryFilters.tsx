...
//                             </div>
//                         ))} */}
//                         // In CategoryFilters component, update the status options
// {[
//   { value: 'active', label: 'Active', color: 'text-green-600' },
//   { value: 'inactive', label: 'Inactive', color: 'text-gray-600' }
// ].map((option) => (
...
const CategoryFilters: React.FC<CategoryFiltersProps> = ({
  ...
  const handleStatusChange = (value: string, checked: boolean | 'indeterminate') => {
    try {
      setFilters(prev => {
        const currentArray = prev.status || [];
        const newArray = checked === true
          ? [...currentArray, value]
          : currentArray.filter(s => s !== value);
