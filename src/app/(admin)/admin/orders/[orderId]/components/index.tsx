...
interface OrderDetailsProps {
  ...
  setClose?: React.Dispatch<React.SetStateAction<boolean>>;
  isModal?: boolean;
  returnPage?: number; // Added for back button functionality
}
...
const OrderDetails: React.FC<OrderDetailsProps> = ({
  ...
  setClose,
  isModal = false,
  returnPage = 1 // Default to page 1
}) => {
  const router = useRouter();
  const [trackingModalOpen, setTrackingModalOpen] = useState(false);
  const [productModalOpen, setProductModalOpen] = useState(false);
  ...
  {profile?.email && (
    ...
    {profile.email}
                ...
                <CardContent className="flex flex-col gap-2 ">
                  <CardTitle> <p className="text-sm text-gray-700 break-words overflow-hidden whitespace-normal max-w-[200px] mx-auto">{profile?.fullName || profile?.businessName || customer?.email || 'Unknown Customer'}</p></CardTitle>
