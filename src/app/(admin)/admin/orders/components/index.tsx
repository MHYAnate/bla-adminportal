...
import { InputFilter } from "@/app/(admin)/components/input-filter1";
import { SelectFilter } from "@/app/(admin)/components/select-filter1";
import { useEffect, useState, useMemo, useCallback } from "react";
import DeleteContent from "@/app/(admin)/components/delete-content";
import {
  Dialog,
...
import { useRouter, useSearchParams } from "next/navigation";
import { ROUTES } from "@/constant/routes";
import { DateRangeFilter } from "@/app/(admin)/components/date-range-filter1";
import { format, subDays } from "date-fns";
import * as XLSX from "xlsx";
...
useEffect(() => {
  ...
  const ordersFilter = useMemo((): OrdersFilter => {
    ...
    if (debouncedFilter && debouncedFilter.trim()) {
      filterObj.search = debouncedFilter.trim();
    }
    ...
    return filterObj;
  // }, [currentPage, pageSize, filter, status, customerType, dateFrom, dateTo, mapStatusToBackend]);
}, [currentPage, pageSize, debouncedFilter, status, customerType, dateFrom, dateTo, mapStatusToBackend]);
