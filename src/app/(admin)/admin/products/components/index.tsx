...
// 			const newItems = getManufacturersData.data.filter(
...
// 				const isSame =
// 					prev.length === getManufacturersData.data.length &&
// 					prev.every(
// 						(m: any, i: number) => m.id === getManufacturersData.data[i].id
// 					);
...
// 				(product) => product.manufacturer?.id?.toString() === manufacturerFilter
// 			);
// 		}
...
export default function Products() {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [filter, status, manufacturerFilter, categoryFilter, startDate, endDate]);
  ...
  // Filter change effect - now includes manufacturer and category filters
  useEffect(() => {
    console.log("🔄 Filter changed, updating with payload:", payload);
    setProductsFilter(payload);
  }, [
        ...
        <CardContent className="p-6">
          ...
          <div className="flex items-center gap-4 mb-6 flex-wrap">
            ...
            {/* <SelectFilter
              ...
              list={[
                { label : "All Manufacturers", value: "all" },
                ...(allManufacturers || []).map((manufacturer: any) => ({
                  label: manufacturer.name,
                  value: manufacturer.id.toString(),
                })),
              ]}
